function F = cauchyKernelMRFF(D,gamma,s,f)
if nargin<4
  f = 20;
end
    %% Modified Random Fourier Feature Embedding for Cauchy Kernel
    ...
    %  from the range [-sqrt(f*gamma^2), sqrt(f*gamma^2)]. 
    %
    % output:
    %
   ...
   gamma = gamma/2;
    ...
    o = length(dgrid);
    pdf = abs(dgrid/gamma).^(d-1).*1./(1 + abs(dgrid)/gamma);
    pdf = pdf/sum(pdf);
